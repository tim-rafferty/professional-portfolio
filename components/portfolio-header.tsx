  ...
  return (
    <header
      className={cn(
        ...
        scrolled ? "bg-zinc-900/90 backdrop-blur-md shadow-md py-1.5" : "bg-transparent",
      )}
    >
      <div className="container mx-auto px-4 flex items-center justify-between">
        {/* Logo/Name */}
        <Link href="/" className="flex items-center group">
          <div className="font-sans font-medium text-white text-xl sm:text-2xl tracking-wide relative overflow-hidden transition-transform duration-300 group-hover:scale-105">
            Timothy Rafferty
            <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-terracotta-400 to-ochre-500 transition-all duration-300 group-hover:w-full"></span>
          </div>
          <span className="font-accent text-zinc-400 text-sm ml-2 hidden sm:inline-block transition-all duration-300 group-hover:text-zinc-300">
            / {personalInfo.title}
          </span>
        </Link>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive ? "text-terracotta-400" : "text-zinc-400 hover:text-white",
                )}
              >
                <span className="relative z-10">{item.label}</span>

                {/* Hover effect - subtle background glow */}
                <span className="absolute inset-0 bg-terracotta-500/10 rounded-md group-hover:bg-terracotta-500/10 transition-all duration-300"></span>

                {/* Hover effect - bottom border */}
                <span
                  className={cn(
                    "absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-gradient-to-r from-terracotta-400 to-ochre-500 transition-all duration-300 group-hover:w-4/5",
                    isActive && "w-4/5",
                  )}
                ></span>
              </Link>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "text-terracotta-400 border-terracotta-400/30"
